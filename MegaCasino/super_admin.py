...
class SuperAdmin(User):
    ...
    def create_casino(self, casino_name: str):
        if len(casino_name) < 2:
            ...
            return
        if casino_name in [current_casino.get_name() for current_casino in self.casinos]:
            print('This casino already exists')
        else:
            casino = Casino(casino_name)
            self.casinos.append(casino)

    def create_game_machine_in_casino(self, game_machine_money: float, casino_name: str):
        for current_casino in self.casinos:
            if casino_name == current_casino.get_name():
                if game_machine_money <= self.get_money():
                    ...
                    current_casino.create_machine(GameMachine(game_machine_money))
                else:
            ...
            if casino_name == casino.get_name():
                if casino.get_money() >= money_to_take:
                    for machine in Casino.casino_machines_dict[casino_name]:
                        if money_to_take == 0:
                            return
                        GameMachine.all_game_machines_priority_queue.remove((machine.get_money() * (-1), machine))
                        if machine.get_money() >= money_to_take:
                            machine.take_money(money_to_take)
                            self.set_money(self.get_money() + money_to_take)
                            heapq.heappush(GameMachine.all_game_machines_priority_queue, (machine.get_money() * (-1), machine))
                            return
                        else:
                            money_to_take -= machine.get_money()
                            self.set_money(self.get_money() + machine.get_money())
    ...
    def put_money_in_machine_by_id(self, game_machine_id: int, money_to_put):
        ...
        if queue_tuple is None:
            print('No game machine with this id found in casinos owned by this super admin')
            return
        found_machine = queue_tuple[0]
        if self.get_money() >= money_to_put:
            # remove the machine from the priority queue, and add it with a new money value
            GameMachine.all_game_machines_priority_queue.remove((found_machine.get_money() * (-1), found_machine))
            self.set_money(self.get_money() - money_to_put)
            found_machine.put_money(money_to_put)
            heapq.heappush(GameMachine.all_game_machines_priority_queue, (found_machine.get_money() * (-1), found_machine))
        else:
    ...
    def delete_game_machine_by_id(self, game_machine_id: int):
        ...
        GameMachine.all_game_machines_priority_queue.remove((found_machine.get_money() * (-1), found_machine))
        Casino.casino_machines_dict[found_casino.get_name()].remove(found_machine)
        del found_machine
        number_of_machines = found_casino.get_machines_count()
        if number_of_machines > 0:
            for machine in Casino.casino_machines_dict[found_casino.get_name()]:
                # remove the machine from the priority queue, and add it with a new money value
                GameMachine.all_game_machines_priority_queue.remove((machine.get_money() * (-1), machine))
                machine.put_money(money_to_split / number_of_machines)
                heapq.heappush(GameMachine.all_game_machines_priority_queue, (machine.get_money() * (-1), machine))
        else:
            # if there are no more machines in the casino, give all money to the casino's owner
